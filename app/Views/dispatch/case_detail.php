...
async function processDispatch() {
    var remark = document.querySelector('#dispatchForm textarea[name="dispatch_remark"]').value;
    if (!remark) { alert('Please enter a remark before completing.'); return; }
    if (!confirm('Mark this lead as completed?')) return;
    var fd = new FormData(document.getElementById('dispatchForm'));
    fd.append('action', 'complete');
    try {
        var r = await fetch('/bestdealcrm/dispatch/cases/process', {
            method: 'POST', body: fd,
        ...
        if (result.success) {
            ...
            setTimeout(function() { window.location.href = '/bestdealcrm/dispatch/cases'; }, 1000);
        } else {
