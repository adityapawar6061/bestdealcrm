...
<div class="page-header d-flex justify-content-between align-items-center">
    ...
    <a href="<?= BASE_URL ?>/dispatch/cases" class="btn btn-outline-secondary btn-sm"><i class="bi bi-arrow-left me-1"></i> Back</a>
</div>
...
<div class="table-container mb-4 p-3">
    <div class="row g-3">
        ...
        <div class="col-md-2"><small class="text-muted d-block">Assigned To</small><strong class="text-primary"><?= htmlspecialchars($assignedAgentName ?? '-') ?></strong></div>
...
<div class="row g-4">
    <div class="col-lg-8">
        ...
        <!-- ===== SECTION 4: UNDERWRITING FORM ===== -->
        <?php if ($uwForm && !empty($uwForm['sections'])): ?>
        <div class="mb-4" style="border-left:4px solid #ffc107;padding-left:0">
            <div class="table-container" style="border-radius:0 12px 12px 0">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h5 class="fw-bold text-warning mb-0">
                        <i class="bi bi-file-earmark-check me-2"></i>4. Underwriting Form
                        <?php if (!empty($uwName)): ?>
                            <small class="fw-normal text-muted ms-2">by <?= htmlspecialchars($uwName) ?></small>
                        <?php endif; ?>
                    </h5>
                    <span class="badge bg-warning text-dark">Read Only</span>
                </div>
                <?php foreach ($uwForm['sections'] as $section): ?>
                    <?php renderFormSectionReadonly($section, $uwFormValues, $lead['id'], $uwName ?? '', 'underwriting'); ?>
                <?php endforeach; ?>
            </div>
        </div>
        <?php else: ?>
        <div class="table-container mb-4" style="border-left:4px solid #ffc107">
            <h5 class="fw-bold text-warning mb-3"><i class="bi bi-file-earmark-check me-2"></i>4. Underwriting Form</h5>
            <div class="alert alert-light mb-0">No underwriting submission found.</div>
        </div>
        <?php endif; ?>

        <!-- ===== SECTION 5: DISPATCH FORM (Editable) ===== -->
        <?php if ($dispatchForm): ?>
        <div class="table-container mb-4">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h5 class="fw-bold text-danger mb-0">
                    <i class="bi bi-truck me-2"></i>5. Dispatch Form (Fill Below)
                </h5>
                <span class="badge bg-danger">Editable</span>
            </div>
            ...
            <form id="dispatchForm" onsubmit="return false;">
                ...
                <?php foreach ($dispatchForm['sections'] as $section): ?>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="row g-3">
                            <?php foreach ($section['fields'] as $field): ?>
                                <?php
                                $value = $dispatchFormValues[$field['id']] ?? $field['default_value'] ?? '';
        ...
        <?php else: ?>
        <div class="alert alert-warning"><i class="bi bi-exclamation-triangle me-1"></i> No Dispatch form configured.</div>
        <?php endif; ?>
    ...
    <div class="col-lg-4">
        ...
        <div class="table-container mb-3">
            <h6 class="fw-bold mb-3">Dispatch Actions</h6>
            <form id="dispatchDecisionForm">
                <?= csrfField() ?>
                <input type="hidden" name="lead_id" value="<?= $lead['id'] ?>">
                <div class="mb-3">
                ...
                <div class="d-grid gap-2">
                    <button type="button" class="btn btn-outline-warning" onclick="saveDispatchDraft()"><i class="bi bi-save me-1"></i> Save Draft</button>
                    <button type="button" class="btn btn-primary" onclick="submitDispatchForm()"><i class="bi bi-send me-1"></i> Submit Dispatch Form</button>
                    <button type="button" class="btn btn-success" onclick="processDispatch()"><i class="bi bi-check-circle me-1"></i> Mark as Completed</button>
                </div>
            </form>
        </div>
    </div>
...
function saveDispatchDraft() {
    ...
    fetch(BASE_URL + '/dispatch/cases/save-draft', {
...
function submitDispatchForm() {
    if (!confirm('Submit dispatch form?')) return;
    var form = document.getElementById('dispatchForm');
    if (!form) return;
    var formData = new FormData(form);
    fetch(BASE_URL + '/dispatch/cases/submit-form', {
        method: 'POST', body: formData, headers: { 'X-Requested-With': 'XMLHttpRequest' }
    }).then(function(r) { return r.json(); })
    .then(function(result) {
        if (result.success) { showToast(result.message, 'success'); }
...
async function processDispatch() {
    var remark = document.querySelector('#dispatchDecisionForm textarea[name="dispatch_remark"]').value;
    if (!remark) { alert('Please enter a remark before completing.'); return; }
    if (!confirm('Mark this lead as completed?')) return;
    var fd = new FormData(document.getElementById('dispatchDecisionForm'));
    fd.append('action', 'complete');
    try {
        var r = await fetch(BASE_URL + '/dispatch/cases/process', {
            method: 'POST', body: fd,
        ...
        if (result.success) {
            ...
            setTimeout(function() { window.location.href = BASE_URL + '/dispatch/cases'; }, 1000);
        } else {
