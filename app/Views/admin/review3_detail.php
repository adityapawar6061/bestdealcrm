...
<div class="page-header d-flex justify-content-between align-items-center">
    <div>
        <h4><i class="bi bi-clipboard2-check me-2"></i>Review Lead #<?= $lead['id'] ?> (Stage 3)</h4>
    ...
    <a href="<?= BASE_URL ?>/admin/review3" class="btn btn-outline-secondary btn-sm"><i class="bi bi-arrow-left me-1"></i> Back</a>
</div>
    ...
    <div class="col-lg-4">
        ...
        <div class="table-container">
            ...
            <form id="reviewForm"><?= csrfField() ?>
                ...
                <div class="mb-3"><label class="form-label small fw-semibold">Admin Approval 3 Remark</label><textarea name="admin_approval3_remark" class="form-control form-control-sm" rows="3" placeholder="Enter remarks..."></textarea></div>
                <div class="mb-3"><label class="form-label small fw-semibold">Assign to Underwriter</label>
                    <select name="assigned_to" class="form-select form-select-sm"><option value="">Select Underwriter</option><?php foreach($underwritingAgents as $a): ?><option value="<?= $a['id'] ?>"><?= htmlspecialchars($a['name']) ?></option><?php endforeach; ?></select>
                </div>
                <div class="d-grid gap-2">
                    <button type="button" class="btn btn-success" onclick="processReview('approve_to_underwriting')"><i class="bi bi-check-lg me-1"></i> Approve & Send to Underwriting</button>
...
async function processReview(action) {
    ...
    try{const r=await fetch('<?= BASE_URL ?>/admin/review3/process',{method:'POST',body:fd,headers:{'X-Requested-With':'XMLHttpRequest'}});const res=await r.json();if(res.success){alert(res.message);window.location.href='<?= BASE_URL ?>/admin/review3';}else{alert(res.error||'Error');}}catch(e){alert('Network error');}
}
</script>
